...
class yinc_call_list:
    def __init__(self):
        ...
        self.file_path = os.path.join(self.pwd_phat, r'苏州白名单通话记录{}.xlsx'.format(
            datetime.datetime.now().strftime("%Y%m%d%H%M%S")))

    # 读取号码清单txt文件
    def phone_list(self):
        phone_list_all = []
        p = open(os.path.join(self.pwd_phat, self.txt_path), 'r').readlines()
        for phone in p:
            phone = phone.replace('\n', '')
    ...
    def smtp_email(self):
        ...
        receiver = ['tangyanqing@jwcredit.cn']  # 收件人邮箱
        cc = []  # 抄送邮箱 'chenwendong@jwcredit.cn'
